...
public abstract class BaseDaoSupport<T extends Serializable, PK extends Serializable> {
     ...
     * @throws IllegalAccessException
     */
    protected int insertAll(List<T> list) throws Exception {
...
//	}
     ...
     * @return 符合条件的所有对象
     */
    protected List<Map<String, Object>> selectBySql(String sql, Object... args) throws Exception {
     ...
     * queryRule.addDescOrder(&quot;id&quot;);
     * queryRule.addAscOrder(&quot;username&quot;);
     * page = userService.select(User.class, queryRule, pageNo, pageSize);
     * </code>
     * </pre>
     *
     * @param queryRule 查询规则
     ...
     * @return 查询出的结果Page
     */
    protected Page<T> select(QueryRule queryRule, final int pageNo, final int pageSize) throws Exception {
        QueryRuleSqlBuilder bulider = new QueryRuleSqlBuilder(queryRule);
        Object[] values = bulider.getValues();
        String ws = removeFirstAnd(bulider.getWhereSql());
        String whereSql = ("".equals(ws) ? ws : (" where " + ws));
...
//		long count = this.jdbcTemplateReadOnly().queryForMap(countSql, param);
     ...
     * @return
     */
    protected Page<Map<String, Object>> selectBySqlToPage(String sql, Object[] param, final int pageNo, final int pageSize) throws Exception {
        String countSql = "select count(1) from (" + sql + ") a";
